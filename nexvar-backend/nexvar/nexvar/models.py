...
from nexvar.scoring import score_sequences, score_sequences_rc
from nexvar.utils import MODEL_NAMES, HF_MODEL_NAME_MAP, CONFIG_MAP

class NexVar:
    def __init__(self, model_name: str = MODEL_NAMES[1], local_path: str = None):
        """
        Load an NexVar checkpoint.
        ...
        Notes:
        NexVar 40b is too large to fit on a single H100 GPU, so needs multiple GPUs.
        ...
        if local_path is not None:
            self.model = self.load_nexvar_model(None, config_path, local_path)
        else:
            self.model = self.load_nexvar_model(model_name, config_path)
        ...
        if return_embeddings:
            if layer_names is None:
                raise ValueError(
                    ...
                    "nexvar_model.model.state_dict().keys() to see available layers."
                )
    ...
    def generate(
        ...
        If force_prompt_threshold is none, sets default assuming 1xH100 (nexvar_7b) and 2xH100 (nexvar_40b) to help avoid OOM errors.
        """
