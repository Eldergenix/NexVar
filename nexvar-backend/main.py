...
app = modal.App("variant-analysis-nexvar", image=nexvar_image)
...
def run_brca1_analysis():
    ...
    print("Loading nexvar model...")
    model = NexVar('nexvar_7b')
    print("NexVar model loaded")

    brca1_df = pd.read_excel(
        '/nexvar/notebooks/brca1/41586_2018_461_MOESM3_ESM.xlsx',
        header=2,
    )
    brca1_df = brca1_df[[
    ...
    with gzip.open('/nexvar/notebooks/brca1/GRCh37.p13_chr17.fna.gz', "rt") as handle:
    ...
    print(
        f'Scoring likelihoods of {len(ref_seqs)} reference sequences with NexVar...')
    ref_scores = model.score_sequences(ref_seqs)

    print(
        f'Scoring likelihoods of {len(var_seqs)} variant sequences with NexVar...')
    ...
    brca1_subset[f'nexvar_delta_score'] = delta_scores

    y_true = (brca1_subset['class'] == 'LOF')
    auroc = roc_auc_score(y_true, -brca1_subset['nexvar_delta_score'])

    # --- Calculate threshold START
    y_true = (brca1_subset["class"] == "LOF")

    fpr, tpr, thresholds = roc_curve(y_true, -brca1_subset["nexvar_delta_score"])
    ...
    lof_scores = brca1_subset.loc[brca1_subset["class"]
                                  == "LOF", "nexvar_delta_score"]
    func_scores = brca1_subset.loc[brca1_subset["class"]
                                   == "FUNC/INT", "nexvar_delta_score"]
    ...
    sns.boxplot(showmeans=True,
                ...
                showcaps=False,
                ax=p)
    plt.xlabel('Delta likelihood score, NexVar')
...
def brca1_example():
    ...
    print("Running BRCA1 variant analysis with NexVar...")
...
class NexVarModel:
    @modal.enter()
    def load_nexvar_model(self):
        from nexvar import NexVar
        print("Loading nexvar model...")
        self.model = NexVar('nexvar_7b')
        print("NexVar model loaded")
...
def main():
    ...
    nexVarModel = NexVarModel()

    url = nexVarModel.analyze_single_variant.web_url
