...
interface ClinVarVariantDetail {
  title: string;
  obj_type?: string;
...
export async function getGenomeChromosomes(genomeId: string) {
  ...
  for (const chromId in chromosomeData.chromosomes) {
    ...
    // chromosomeData.chromosomes is Record<string, number>, so accessing by chromId is safe
    ...
    for (let i = 0; i < Math.min(10, data[0]); ++i) {
      if (i < data[3].length) {
        try {
          ...
          // We know display exists because of the i < data[3].length check (if consistent)
          // but TS might not ensure it.
          if (!display) continue;
...
export async function fetchGeneDetails(geneId: string): Promise<{
  ...
  try {
    ...
    if (detailData.result?.[geneId]) {
      const detail = detailData.result[geneId] as NCBIGeneDetail;
...
export async function fetchClinvarVariants(
  ...
  if (summaryData.result?.uids) {
    for (const id of summaryData.result.uids) {
      const variant = summaryData.result[id] as ClinVarVariantDetail; // Safe cast inside loop
      variants.push({
        clinvar_id: id,
        title: variant.title,
