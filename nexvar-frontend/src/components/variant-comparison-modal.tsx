...
export function VariantComparisonModal({
  ...
  if (!comparisonVariant || !comparisonVariant.nexVarResult) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white">
        ...
        {/* Modal content */}
        <div className="p-5">
          {comparisonVariant && comparisonVariant.nexVarResult && (
                ...
                <div className="rounded-md border border-[#3c4f3d]/10 bg-white p-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="rounded-md bg-[#e9eeea]/50 p-4">
                      ...
                      <div className="mt-2">
                        <div
                          className={`flex w-fit items-center gap-1 rounded-md px-2 py-1 text-xs font-normal ${getClassificationColorClasses(comparisonVariant.nexVarResult.prediction)}`}
                        >
                          <Shield className="h-3 w-3" />
                          {comparisonVariant.nexVarResult.prediction}
                        </div>
                      </div>
                      {/* Delta score */}
                      <div className="mt-3">
                        ...
                        <div className="text-sm font-medium">
                          {comparisonVariant.nexVarResult.delta_score.toFixed(6)}
                        </div>
                        <div className="text-xs text-[#3c4f3d]/60">
                          {comparisonVariant.nexVarResult.delta_score < 0
                      ...
                      <div className="mt-3">
                        ...
                        <div className="mt-1 h-2 w-full rounded-full bg-[#e9eeea]/80">
                          <div
                            className={`h-2 rounded-full ${comparisonVariant.nexVarResult.prediction.includes("pathogenic") ? "bg-red-600" : "bg-green-600"}`}
                            style={{
                              width: `${Math.min(100, comparisonVariant.nexVarResult.classification_confidence * 100)}%`,
                            }}
                          ></div>
                        </div>
                        <div className="mt-1 text-right text-xs text-[#3c4f3d]/60">
                          {Math.round(
                            comparisonVariant.nexVarResult
                  ...
                  <div className="mt-4 rounded-md bg-[#e9eeea]/20 p-3 text-xs leading-relaxed">
                    <div className="flex items-center gap-2">
                      <span
                        className={`flex h-5 w-5 items-center justify-center rounded-full ${comparisonVariant.classification.toLowerCase() === comparisonVariant.nexVarResult.prediction.toLowerCase() ? "bg-green-100" : "bg-yellow-100"}`}
                      >
                        {comparisonVariant.classification.toLowerCase() ===
                        comparisonVariant.nexVarResult.prediction.toLowerCase() ? (
                      ...
                      <span className="font-medium text-[#3c4f3d]">
                        {comparisonVariant.classification.toLowerCase() ===
                        comparisonVariant.nexVarResult.prediction.toLowerCase()
                          ? "NexVar prediction agrees with ClinVar classification"
                          : "NexVar prediction differs from ClinVar classification"}
                      </span>
                    </div>
                  </div>
                </div>
