  ...
  const analyzeVariant = async (variant: ClinvarVariant) => {
    ...
    try {
      ...
      updateClinvarVariant(variant.clinvar_id, {
        ...variant,
        isAnalyzing: false,
        nexVarError: error instanceof Error ? error.message : "Analysis failed",
      });
    }
  };
  return (
                ...
                {clinvarVariants.map((variant) => (
                  ...
                  >
                    ...
                    <TableCell className="py-2 text-xs">
                      ...
                      {variant.nexVarResult && (
                        <div className="mt-2">
                          <div
                            className={`flex w-fit items-center gap-1 rounded-md px-2 py-1 text-center ${getClassificationColorClasses(variant.nexVarResult.prediction)}`}
                          >
                            <Shield className="h-3 w-3" />
                            <span>NexVar: {variant.nexVarResult.prediction}</span>
                          </div>
                        </div>
                      )}
                    </TableCell>
